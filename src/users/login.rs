...
pub fn login_user(conn: &mut SqliteConnection, username_input: &str, password_input: &str) -> Result<User, FastingAppError> {
    let user = users
...
pub fn login_user_or_device(
    ...
    if let Some(device_id_value) = device_id_input {
        return users
            ...
            .optional()
            .map_err(FastingAppError::DatabaseError)?
            .ok_or_else(|| FastingAppError::InvalidCredentials(format!("Device ID '{}' not found", device_id_value)));
    }
    ...
    Err(FastingAppError::InvalidRequest("Must provide either device ID or username/password.".to_string()))
}
